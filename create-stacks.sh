...
do 
  SUB_STACK_EXISTS=$(aws cloudformation describe-stacks --stack-name "${STACK_NAME}" --region "${region}" 2>/dev/null)
  if [ -z "${SUB_STACK_EXISTS}" ]; then
  ...
  echo "${MAIN_ROUTE_TABLE}"
  echo "${VPC_CIDR_BLOCK}"
  echo "${VPC_PEERING_CONNECTION}"
